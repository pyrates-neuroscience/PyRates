...
def david_sigmoid(membrane_potential,
                  max_firing_rate,
                  membrane_potential_threshold,
                  sigmoid_steepness,
                  membrane_potential_scaling):
    """
    ...
    """
    return max_firing_rate / (1 + np.exp(sigmoid_steepness * (membrane_potential_threshold -
                                                              membrane_potential * membrane_potential_scaling)))


class DavidAxon(Axon):
    """Sigmoidal axon as defined in [1]_.
    ...
    membrane_potential_threshold
        Default = 0.001 V. See documentation of parameter 'membrane_potential_threshold' of :class:`SigmoidAxon`.
   membrane_potential_scaling
        Default = 0. Multiplied to incoming membrane potential of transfer function.
    ...
    def __init__(self,
                 ...
                 membrane_potential_threshold: float = 6e-3,
                 membrane_potential_scaling: float = 1.
                 ) -> None:
        """Instantiate david axon."""

        super().__init__(david_sigmoid,
                         'David_Axon',
                         max_firing_rate=max_firing_rate,
                         membrane_potential_threshold=membrane_potential_threshold,
                         sigmoid_steepness=sigmoid_steepness,
                         membrane_potential_scaling=membrane_potential_scaling)
