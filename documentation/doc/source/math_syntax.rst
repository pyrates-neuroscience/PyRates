...
- :code:`past`: function call that should be used to implement delayed DEs. The function :code:`past` takes two arguments - the first one is the state variable and the second one is the constant delay. As an example, :code:`past(x, tau)` implements :math:`x(t-\tau)`. As a convenient shorthand, :code:`x(t-tau)` anywhere in an equation string is automatically rewritten to :code:`past(x, tau)` by PyRates, so both forms are accepted.
