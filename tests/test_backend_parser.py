...
def test_1_7_equation_parsing():
    ...
    for eq, args, target in zip(equations, arguments, results):
        ...
        result_tmp = parse_equation_system(node_equations=[(eq, 'node/op')], equation_args=args, backend=b)['node/op/a']
    ...
    for eq, args, target in zip(equations, arguments, results):
        # tensorflow-based parsing
        result = parse_equation_system(node_equations=[(eq, 'node/op')], equation_args=args, backend=b)['node/op/a']
        #assert result == pytest.approx(target, rel=1e-6)
