...
def test_2_1_operator():
    ...
    for b in backends:
        ...
        net = ComputeGraph(net_config=net_config, name='net3', vectorization=True, dt=dt, backend=b)
        ...
        diff = results['b'].values[1:] - targets[:-1, 1]
        assert np.mean(np.abs(diff)) == pytest.approx(0., rel=1e-6, abs=1e-6)
...
def test_2_2_node():
    ...
    for b in backends:
        ...
        diff = np.mean(np.abs(results['a'].values[1:] - targets[:-1, 1])) + \
               np.mean(np.abs(results['b'].values[1:] - targets[:-1, 3]))
